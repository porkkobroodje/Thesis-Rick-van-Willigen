...
create_constraints() #creating all constraint via a function
#%% optimzing parameters (step 1)
   
#termination callback for when enough feasible solutions are reached.
...
#%% Step 2,3,4 of the Algorithm 
genes_population_l = []
score_population_l =[]


#Function te create a crossoverpoint between 2 solutions and merge merge the 2 solutions.
...
#Function for creating new chromosomes(solutions) and mutating variables.
def Create_new_chromosome_and_mutate(genes_population_a,fitness_population):
...
#retrieving all variables, and solution out of the solution pool.
...
#start step 2,3,4
while generation < 10:  #termination criteria
    ...
    if model.status != 3 : #feasible solution check
    ...
    #clearing old constraints and creating the original model             
